  ...
  return (
    <div className="relative mx-auto w-full max-w-4xl px-4 py-8">
      <motion.div
        ...
        transition={{ duration: 0.4, repeat: isComplete ? 2 : 0 }}
        className="relative h-12 w-full overflow-hidden rounded-2xl border border-white/20 bg-slate-950/40 p-1 shadow-[0_0_25px_rgba(0,0,0,0.5)] backdrop-blur-xl"
      >
        {/* Progress Fill */}
        <motion.div
          initial={{ width: "25%" }}
          animate={{ width: `${level * 25}%` }}
          transition={{ duration: 1, ease: "circOut" }}
          className="relative h-full rounded-xl bg-[linear-gradient(90deg,#1e40af_0%,#3b82f6_50%,#22d3ee_100%)] shadow-[0_0_20px_rgba(34,211,238,0.5)]"
        >
          {/* Shiny Light Sweep */}
          <motion.div
            animate={{ x: ["-100%", "250%"] }}
        ...
        {/* Level Markers & Labels Inside Bar */}
        <div className="absolute inset-0 flex items-center justify-around">
          {steps.map((text, i) => (
            <div key={i} className="relative flex flex-col items-center justify-center">
              <p className={`text-center text-[8px] font-black uppercase tracking-tighter sm:text-[10px] ${level >= i + 1 ? "text-white" : "text-white/20"}`}>
                {text}
              </p>
              {/* The 3 Line Dividers */}
              {i < 3 && (
                <div className="absolute -right-12 h-6 w-[1px] bg-white/10 sm:-right-20 md:-right-24" />
              )}
            </div>
          ))}
        </div>
